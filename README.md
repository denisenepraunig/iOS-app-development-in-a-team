...
## Git and GitHub
The current standard seems to be `git` and GitHub is a convenient way to host your source code. There are enterprise options available, e.g. to sign-in with your companies Identity Provider. One great benefit of GitHub are their GitHub actions which can be used to automate tasks, like building and testing apps. 
...
Based on your teams working style it may be a good idea to protect the main branch - so no one can directly commit to it. However I've also worked on a rather small team without any review process were everything was directly commited to main and you just fetched and merged.
...
You can setup templates for your Pull Request, this helps tremendously becaue every Pull Request looks the same and nothing is forgotten. This can include a link to your Jira Ticket, Screenshots, How to Test and so on.
...
When reviewing Pull Request we are trying to find errors or spot mistakes. If I receive a review which only highlights my mistakes, I feel a bit down. Therefore I also try to point out some things like "Wow, that's a smart use case of a Set here" or "TIL (today-I-learned)" or put some 🎉 emoji into the review. Sometimes I comment my own Pull Request and say "@Team member thanks for this cool idea" (which came up during pairing or so).
